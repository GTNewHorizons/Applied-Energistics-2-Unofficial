...
public class ContainerPatternTermEx extends ContainerMEMonitorable implements IOptionalSlotHost, IContainerCraftingPacket {
    ...
    public ContainerPatternTermEx(final InventoryPlayer ip, final ITerminalHost monitorable )
    {
        ...
        this.addSlotToContainer( this.patternSlotLoad = new SlotRestrictedInput( SlotRestrictedInput.PlacableItemType.ENCODED_PATTERN, patternInv, 2, 90, -29, this.getInventoryPlayer() ) );
    ...
    private void offsetSlots(boolean initial) {
        ...
        int loadPatternOffset = 18 * 3;
        if (initial) {
            if (inverted) {
                patternSlotLoad.xDisplayPosition -= loadPatternOffset;
            }
        } else {
            if (inverted) {
                patternSlotLoad.xDisplayPosition -= loadPatternOffset;
