...
@InterfaceList(
        value = {
                @Interface(
                        iname = IntegrationType.RotaryCraft,
                        iface = "Reika.RotaryCraft.API.Power.AdvancedShaftPowerReceiver"),
                @Interface(
                        iname = IntegrationType.RotaryCraft,
                        iface = "Reika.RotaryCraft.API.Interfaces.Transducerable") })
public abstract class RotaryCraft extends IC2 implements AdvancedShaftPowerReceiver, Transducerable {
    ...
    @Override
    public final ArrayList<String> getMessages(final World world, final int x, final int y, final int z,
            final int side) {
        final String out;
